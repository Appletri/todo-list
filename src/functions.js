...
function updateContent(arr, filterTrue) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    ...
    div.innerHTML = `<div class='priority' id='${arr[i].id}p'></div> <input id='${arr[i].id}cb' class='checkbox' type='checkbox'> <p>${arr[i].task} <p>${arr[i].dueDate} <p id='${arr[i].id}time'>${timeConversion(arr[i].time)}`;
    if (filterTrue === true) {
  ...
  if (taskIn.value !== '' || ddIn.value !== '' || selProject !== null || selProject !== 'Today' || selProject !== 'Week' || selProject !== 'All') {
    for (let i = 0; i < projects.length; i += 1) {
      if (projects[i].name === selProject) {
        const newTask = new Task(taskIn.value, ddIn.value, time.value, projects[i].tasks);
        newTask.assignId();
...
function updatePriority(id, time, date) {
  ...
  if (currentW >= 0) {
    ...
    document.getElementById(`${id}p`).style.filter = `grayscale(${currentW * -1}%)`;
  }
}
...
export {
  createList, addNavFunc, setupInput, updateContent, addProjectButton,
};
